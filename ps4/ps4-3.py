...
def mcmc(pars,step_size,y,fun,npts,nstep=1000,noise=None):
    ...
    for i in range(nstep):
        ...
        accept=False
        if np.random.rand(1)<accept_prob and trial_pars[3]>0: #force tau to be > 0
            accept=True
            count = count + 1
...
start_t = time.time()
chain, accept_percentage = mcmc(pars=pars_guess, step_size=L, y=spec, fun=spectrum_chisq, npts=len(spec), nstep=1500, noise=errs)
